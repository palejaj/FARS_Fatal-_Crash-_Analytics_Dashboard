...
# Load and save filtered datasets once (can be skipped on future runs)
if not all([os.path.exists(f) for f in ["accidents_filtered.csv", "person_filtered.csv", "drugs_filtered.csv"]]):
    accidents = pd.read_csv("fars_accidents_1975_2023_cleaned.csv", low_memory=False)
    accidents = accidents[accidents["YEAR"].between(2008, 2023)]
    accidents.to_csv("accidents_filtered.csv", index=False)

    cols_person = ["YEAR", "ST_CASE", "AGE", "SEXNAME"]
    person = pd.read_csv("fars_person_2008_2023_combined.csv", low_memory=False, usecols=cols_person)
    person = person[person["YEAR"].between(2008, 2023)]
    person.to_csv("person_filtered.csv", index=False)

    cols_drugs = ["YEAR", "ST_CASE"]
    drugs = pd.read_csv("fars_drugs_2008_2023_combined.csv", low_memory=False, usecols=cols_drugs)
    drugs = drugs[drugs["YEAR"].between(2008, 2023)]
    drugs.to_csv("drugs_filtered.csv", index=False)
